#include <usb1608/usb1608.h>
#include <unistd.h>

USB1608::USB1608(std::shared_ptr<rclcpp::Node> nh, std::string daq_id)
    : nh_(nh), daq_id_(daq_id)
{
  daq_ready_ = false;
  daq_connected_ = false;
  ai_enabled_ = false;
  dio_enabled_ = false;
  n_dinput_ = 0;
  n_doutput_ = 0;

  descriptorIndex_ = 0;
  interfaceType_ = USB_IFC;
  daqDeviceHandle_ = 0;
  numDevs_ = kMaxDevCount;

  ai_state_.resize(kNumberOfAiChannels);
  di_state_.resize(kNumberOfDioIO);
  flags_ = AIN_FF_DEFAULT;

  // Scan options AI
  numberOfChannels_ = 0;
  ai_rate_ = 10000;
  ai_scanOptions_ = (ScanOption)(SO_DEFAULTIO | SO_CONTINUOUS);
  flags_ai_scan_ = AINSCAN_FF_DEFAULT;
  samplesPerChannel_ = 10;

  int i = 0;
  err_ = ERR_NO_ERROR;
  DaqDeviceDescriptor devDescriptors[kMaxDevCount];

  err_ = ulGetDaqDeviceInventory(interfaceType_, devDescriptors, &numDevs_);
  if (err_ != ERR_NO_ERROR)
  {
    RCLCPP_ERROR(nh_->get_logger(), "Error getting DAQ boards information: %d", err_);
    return;
  }

  // verify at least one DAQ device is detected
  if (numDevs_ == 0)
  {
    RCLCPP_ERROR(nh_->get_logger(), "No DAQ device is detected");
    return;
  }

  bool found = false;
  for (i = 0; i < (int)numDevs_; i++)
  {
    if (devDescriptors[i].uniqueId == daq_id_)
    {
      devDescriptor_ = devDescriptors[i];
      found = true;
      break;
    }
  }

  if (!found)
  {
    RCLCPP_ERROR(nh_->get_logger(), "DAQ Board with ID %s not found between %d devices found", daq_id_.c_str(), numDevs_);
    return;
  }

  // get a handle to the DAQ device associated with the first descriptor
  RCLCPP_INFO(nh_->get_logger(), "Creating DAQ Handle for ID %s", daq_id_.c_str());
  daqDeviceHandle_ = ulCreateDaqDevice(devDescriptor_);
  if (daqDeviceHandle_ == 0)
  {
    RCLCPP_ERROR(nh_->get_logger(), "Unable to create a handle for %s DAQ device", daq_id_.c_str());
    return;
  }

  daq_ready_ = true;

  // Filters coefficients
  VectorXd a_coef(3);
  a_coef << -2.7488358092146754, 2.5282312191425587, -0.7776385602380801;

  VectorXd b_coef(4);
  b_coef << 0.00021960621122536214, 0.0006588186336760865,
      0.0006588186336760865, 0.00021960621122536214;

  filter_iir_ai = new filters::IIR(16, 3, b_coef, a_coef);

  // Publishers
  pub_ai_state_ = nh_->create_publisher<std_msgs::msg::Float64MultiArray>("/usb1608/ai/state", 1);
  pub_ai_state_filtered = nh_->create_publisher<std_msgs::msg::Float64MultiArray>("/usb1608/ai/state_filtered", 1);
  pub_di_state_ = nh_->create_publisher<std_msgs::msg::Int32MultiArray>("/usb1608/di/state", 1);

  // Subscribers
  sub_do_cmd_ = nh_->create_subscription<std_msgs::msg::Int32MultiArray>(
      "/usb1608/do/cmd", 1000, std::bind(&USB1608::UpdateDOValueCb, this, std::placeholders::_1));

  pub_ai_state_msg_.data.resize(kNumberOfAiChannels);
  pub_ai_state_filtered_msg_.data.resize(kNumberOfAiChannels);
  pub_di_state_msg_.data.resize(kNumberOfDioIO);

  ai_state_filtered_.resize(kNumberOfAiChannels);
}

USB1608::~USB1608() {
    if (filter_iir_ai) delete filter_iir_ai;
}

int USB1608::InitAI()
{
  int hasAI = 0;
  char inputModeStr[64];
  char rangeStr[64];

  if (!daq_ready_)
  {
    RCLCPP_ERROR(nh_->get_logger(), "DAQ board is not ready");
    return -1;
  }

  err_ = getDevInfoHasAi(daqDeviceHandle_, &hasAI);
  if (!hasAI)
  {
    RCLCPP_ERROR(nh_->get_logger(), "The specified DAQ device does not support analog input");
    return -1;
  }

  RCLCPP_WARN(nh_->get_logger(), "Connecting to device %s - please wait ...", devDescriptor_.devString);

  int attempts = 10;
  for (int trial = 0; trial < attempts; trial++)
  {
    err_ = ulConnectDaqDevice(daqDeviceHandle_);
    if (err_ == ERR_NO_ERROR)
    {
      RCLCPP_INFO(nh_->get_logger(), "Connected to device %s", devDescriptor_.devString);
      break;
    }
    else
    {
      RCLCPP_WARN(nh_->get_logger(), "Connection to device %s failed, attempt %d of %d", devDescriptor_.devString, trial + 1, attempts);
      if (trial == attempts - 1)
      {
        RCLCPP_ERROR(nh_->get_logger(), "Connection to device %s failed, exiting", devDescriptor_.devString);
        return -1;
      }
      sleep(1);
    }
  }

  daq_connected_ = true;

  err_ = getAiInfoFirstSupportedInputMode(daqDeviceHandle_, &numberOfChannels_, &inputMode_, inputModeStr);
  err_ = getAiInfoFirstSupportedRange(daqDeviceHandle_, inputMode_, &ai_range_, rangeStr);

  RCLCPP_INFO(nh_->get_logger(), "ANALOG INPUT INFO");
  RCLCPP_INFO(nh_->get_logger(), "\t# Channels: %d", numberOfChannels_);
  RCLCPP_INFO(nh_->get_logger(), "\tInput mode: %s", inputModeStr);
  RCLCPP_INFO(nh_->get_logger(), "\tRange: %s", rangeStr);

  ai_enabled_ = true;
  return 0;
}

int USB1608::InitDIO()
{
  InitDIO(kNumberOfDioIO, 0);
  return 0;
}

int USB1608::InitDIO(int n_din, int n_dout)
{
  int hasDIO = 0;
  char portTypeStr[kMaxStrlength];
  char portIoTypeStr[kMaxStrlength];

  if (!daq_ready_)
  {
    RCLCPP_ERROR(nh_->get_logger(), "DAQ board is not ready");
    return -1;
  }

  if (n_din + n_dout != kNumberOfDioIO)
  {
    RCLCPP_ERROR(nh_->get_logger(), "Requested DIN + DOUT doesn't match device number of DIOs");
    return -1;
  }

  n_dinput_ = n_din;
  n_doutput_ = n_dout;

  err_ = getDevInfoHasDio(daqDeviceHandle_, &hasDIO);
  if (!hasDIO)
  {
    RCLCPP_ERROR(nh_->get_logger(), "The specified DAQ device does not support Digital I/O");
    return -1;
  }

  if (!daq_connected_)
    err_ = ulConnectDaqDevice(daqDeviceHandle_);

  if (err_ != ERR_NO_ERROR)
  {
    RCLCPP_ERROR(nh_->get_logger(), "ERROR connecting to DAQ device");
    return -1;
  }

  daq_connected_ = true;

  err_ = getDioInfoFirstSupportedPortType(daqDeviceHandle_, &portType_, portTypeStr);
  err_ = getDioInfoFirstSupportedPortIoType(daqDeviceHandle_, &portIoType_, portIoTypeStr);
  err_ = getDioInfoNumberOfBitsForFirstPort(daqDeviceHandle_, &bitsPerPort_);

  if (n_din == kNumberOfDioIO)
  {
    if (portIoType_ == DPIOT_IO || portIoType_ == DPIOT_BITIO)
    {
      err_ = ulDConfigPort(daqDeviceHandle_, portType_, DD_INPUT);
      RCLCPP_INFO(nh_->get_logger(), "DIO Port configure as INPUT");
    }
  }
  else if (n_din == 0)
  {
    if (portIoType_ == DPIOT_IO || portIoType_ == DPIOT_BITIO)
    {
      err_ = ulDConfigPort(daqDeviceHandle_, portType_, DD_OUTPUT);
      RCLCPP_INFO(nh_->get_logger(), "DIO Port configure as OUTPUT");
    }
  }
  else
  {
    if (portIoType_ == DPIOT_BITIO)
    {
      RCLCPP_WARN(nh_->get_logger(), "\tAssigning first %i bits as INPUT and next %i bits as OUTPUT", n_din, bitsPerPort_ - n_din);
      for (int bitNumber = 0; bitNumber < bitsPerPort_; bitNumber++)
      {
        if (bitNumber < n_din)
          err_ = ulDConfigBit(daqDeviceHandle_, portType_, bitNumber, DD_INPUT);
        else
          err_ = ulDConfigBit(daqDeviceHandle_, portType_, bitNumber, DD_OUTPUT);

        if (err_ != ERR_NO_ERROR) break;
      }
    }
    else
    {
      RCLCPP_WARN(nh_->get_logger(), "Device doesn't support bit IO assignment. Setting entire port as INPUT");
      err_ = ulDConfigPort(daqDeviceHandle_, portType_, DD_INPUT);
    }
  }

  RCLCPP_INFO(nh_->get_logger(), "DIO PORT INFO");
  RCLCPP_INFO(nh_->get_logger(), "\tPort: %s", portTypeStr);
  RCLCPP_INFO(nh_->get_logger(), "\tPort I/O type: %s", portIoTypeStr);

  dio_enabled_ = true;
  RCLCPP_INFO(nh_->get_logger(), "\tDIO ENABLED: %i", dio_enabled_);
  return 0;
}

int USB1608::StartScanAI()
{
  if (!ai_enabled_)
  {
    RCLCPP_ERROR(nh_->get_logger(), "AI Module is not enabled");
    return -1;
  }

  ai_buffer_ = (double *)malloc(numberOfChannels_ * samplesPerChannel_ * sizeof(double));
  err_ = ulAInScan(daqDeviceHandle_, 0, numberOfChannels_ - 1, inputMode_,
                   ai_range_, samplesPerChannel_, &ai_rate_, ai_scanOptions_,
                   flags_ai_scan_, ai_buffer_);

  if (err_ == ERR_NO_ERROR)
    return 0;

  return -1;
}

int USB1608::StopScanAI()
{
  ScanStatus status;
  TransferStatus transferStatus;

  err_ = ulAInScanStatus(daqDeviceHandle_, &status, &transferStatus);

  if (status == SS_RUNNING && err_ == ERR_NO_ERROR)
  {
    err_ = ulAInScanStop(daqDeviceHandle_);
  }

  if (ai_buffer_) {
      free(ai_buffer_);
      ai_buffer_ = NULL;
  }

  return 0;
}

int USB1608::UpdateStateAI()
{
  double data = 0;

  if (!ai_enabled_)
  {
    RCLCPP_ERROR(nh_->get_logger(), "AI Module is not enabled");
    return -1;
  }

  for (int chan = 0; chan < numberOfChannels_; chan++)
  {
    err_ = ulAIn(daqDeviceHandle_, chan, inputMode_, ai_range_, flags_, &data);
    if (err_ == ERR_NO_ERROR)
    {
      ai_state_.at(chan) = data;
    }
  }
  return 0;
}

int USB1608::UpdateScanStateAI()
{
  ScanStatus status;
  TransferStatus transferStatus;

  err_ = ulAInScanStatus(daqDeviceHandle_, &status, &transferStatus);
  int index = transferStatus.currentIndex;

  for (int ch = 0; ch < numberOfChannels_; ch++)
  {
    if (err_ == ERR_NO_ERROR)
    {
      ai_state_.at(ch) = ai_buffer_[index + ch];
    }
  }

  ai_state_filtered_ = filter_iir_ai->update(VectorXd::Map(&ai_state_[0], 16));

  return 0;
}

std::vector<double> USB1608::GetScanStateChannelsAI(int start_ch, int n_channels)
{
  ScanStatus status;
  TransferStatus transferStatus;
  std::vector<double> ai_out;

  ai_out.resize(n_channels);

  err_ = ulAInScanStatus(daqDeviceHandle_, &status, &transferStatus);
  int index = transferStatus.currentIndex;

  for (int ch = start_ch; ch < start_ch + n_channels; ch++)
  {
    if (err_ == ERR_NO_ERROR)
    {
      ai_out.at(ch - start_ch) = ai_buffer_[index + ch];
    }
  }

  return ai_out;
}

int USB1608::UpdateStateDI()
{
  unsigned int data = 0;

  if (!dio_enabled_)
  {
    RCLCPP_ERROR(nh_->get_logger(), "DI Module is not enabled");
    return -1;
  }

  for (int bitNumber = 0; bitNumber < bitsPerPort_; bitNumber++)
  {
    err_ = ulDBitIn(daqDeviceHandle_, portType_, bitNumber, &data);
    if (err_ == ERR_NO_ERROR)
    {
      di_state_.at(bitNumber) = data;
    }
  }

  return 0;
}

int USB1608::UpdateStateDI(int bit_number)
{
  unsigned int data = 0;

  if (!dio_enabled_)
  {
    RCLCPP_ERROR(nh_->get_logger(), "DI Module is not enabled");
    return -1;
  }

  err_ = ulDBitIn(daqDeviceHandle_, portType_, bit_number, &data);
  RCLCPP_INFO(nh_->get_logger(), "Bit Number: %d:  Data: %d", bit_number, data);
  return 0;
}

int USB1608::UpdateStateDO(int bit_number, unsigned int bit_value)
{
  if (!dio_enabled_)
  {
    RCLCPP_ERROR(nh_->get_logger(), "DIO Module is not enabled");
    return -1;
  }

  err_ = ulDBitOut(daqDeviceHandle_, portType_, bit_number, bit_value);
  return 0;
}

int USB1608::UpdateStateDO()
{
  if (!dio_enabled_)
  {
    RCLCPP_ERROR(nh_->get_logger(), "DIO Module is not enabled");
    return -1;
  }
  if (n_doutput_ > 0 && (int)do_cmd_.size() == n_doutput_)
  {
    for (int bitNumber = 0; bitNumber < n_doutput_; bitNumber++)
    {
      err_ = ulDBitOut(daqDeviceHandle_, portType_, n_dinput_ + bitNumber, do_cmd_.at(bitNumber));
    }
  }
  else
  {
    RCLCPP_ERROR(nh_->get_logger(), "No DO enabled OR Command size doesn't match number of DOs enabled");
  }
  return 0;
}

int USB1608::PublishStateAI()
{
  pub_ai_state_msg_.data = ai_state_;
  pub_ai_state_->publish(pub_ai_state_msg_);
  
  for(size_t i=0; i < (size_t)ai_state_filtered_.size(); ++i) {
      pub_ai_state_filtered_msg_.data[i] = ai_state_filtered_[i];
  }
  pub_ai_state_filtered->publish(pub_ai_state_filtered_msg_);
  return 0;
}

int USB1608::PublishStateDI()
{
  pub_di_state_msg_.data.clear();
  for(int val : di_state_) pub_di_state_msg_.data.push_back(val);
  pub_di_state_->publish(pub_di_state_msg_);
  return 0;
}

void USB1608::Quit()
{
  ulDisconnectDaqDevice(daqDeviceHandle_);
  if (daqDeviceHandle_)
    ulReleaseDaqDevice(daqDeviceHandle_);
}

void USB1608::PrintError(UlError err_)
{
  if (err_ != ERR_NO_ERROR)
  {
    char errMsg[ERR_MSG_LEN];
    ulGetErrMsg(err_, errMsg);
    RCLCPP_ERROR(nh_->get_logger(), "Error Code: %d", err_);
    RCLCPP_ERROR(nh_->get_logger(), "Error Message: %s", errMsg);
  }
}

void USB1608::UpdateDOValueCb(const std_msgs::msg::Int32MultiArray::SharedPtr msg)
{
  RCLCPP_INFO(nh_->get_logger(), "Updating DO");
  do_cmd_ = msg->data;
  UpdateStateDO();
}

int USB1608::get_ai_channels() { return numberOfChannels_; }
int USB1608::get_di_channels() { return n_dinput_; }
int USB1608::get_do_channels() { return n_doutput_; }